...
import { eleves, CLASSES, isNouvel } from '@/lib/data'
...
export default function PresencesElevesPage() {
  ...
  const nbAbsents = elevesClasse.length - nbPresents
          ...
          <div className="p-4">
            ...
            {classe && (
              <div className="flex gap-1 mb-4">
                {['Lun 07', 'Mar 08', 'Mer 09', 'Jeu 10', 'Ven 11'].map((j, i) => (
                  <div key={j} className={`flex-1 text-center py-2 rounded-lg text-[10px] font-medium cursor-pointer ${i === 0 ? 'bg-[#2d6a2d] text-white' : 'bg-gray-100 text-gray-500'}`}>
                    {j}
                  </div>
                ))}
              </div>
            ...
            {classe && elevesClasse.length > 0 && (
              <div className="flex gap-2 mb-3">
                <span className="text-[10px] bg-green-50 text-green-700 font-medium px-2 py-1 rounded-full">
                  {nbPresents} présents
                </span>
                <span className="text-[10px] bg-red-50 text-red-700 font-medium px-2 py-1 rounded-full">
                  {nbAbsents} absents
                </span>
                ...
                return (
                  <div key={c}>
                    <div className="flex justify-between text-xs mb-1">
                      ...
                      <span className="text-gray-500">{Math.round(nb * pct / 100)}/{nb} — {pct}%</span>
                    </div>

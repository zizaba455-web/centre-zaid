      ...
      {tab === 'pointage' && (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
          <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            ...
            <span className="text-xs text-gray-400">{profs.filter(p => p.status === 'present').length}/{profs.length} présents</span>
          </div>
          <div className="divide-y divide-gray-50">
            {profs.map((prof, i) => (
              <div key={prof.id} className="flex items-center gap-4 px-5 py-4 flex-wrap">
                ...
                <div className="text-xs text-gray-400 w-28 text-center">{prof.signIn ? `Arrivée: ${prof.signIn}` : '—'}</div>
                <div className="text-xs text-gray-400 w-28 text-center">{prof.signOut ? `Départ: ${prof.signOut}` : '—'}</div>
                <button onClick={() => signIn(prof.id)} disabled={!!prof.signIn}
                ...
                <button onClick={() => signOut(prof.id)} disabled={!prof.signIn || !!prof.signOut}

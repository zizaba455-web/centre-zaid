...
import { getEleves, CLASSES, POLES, isNouvel, getInitiales, AV_COLORS, getMontant } from '@/lib/data'
...
export default function ElevesPage() {
  ...
  useEffect(() => {
    getEleves().then(data => { setEleves(data); setLoading(false) })
  }, [])
  ...
  async function changeStatus(id: number, status: StatutPaiement) {
    await supabase.from('eleves').update({ status }).eq('id', id)
    setEleves(prev => prev.map(e => e.id === id ? { ...e, status } : e))
  }

  async function deleteEleve(id: number) {
  ...
  async function saveEdit(updated: Eleve) {
    await supabase.from('eleves').update({
      prenom: updated.prenom, nom: updated.nom,
      classe: updated.classe, pole: updated.pole,
      tuteur: updated.tuteur, tel: updated.tel, adresse: updated.adresse
    }).eq('id', updated.id)
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-400 mt-1">{filtered.length} élève(s) — données en direct ✓</p>
      </div>
      ...
      <div className="flex gap-3 mb-5 flex-wrap">
        ...
        />
        <select className="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#2d6a2d]"
                ...
                <tr key={e.id} className="border-t border-gray-50 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-1">
                      <button onClick={() => { setSelected(e); setShowEdit(true) }}
                        className="text-[10px] bg-gray-100 hover:bg-gray-200 px-2 py-1 rounded">✏</button>
                      <button onClick={() => deleteEleve(e.id)}
                        className="text-[10px] bg-red-50 hover:bg-red-100 text-red-600 px-2 py-1 rounded">✕</button>
                    </div>
                  </td>
                </tr>
              ))}
    ...
    <div>
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="flex flex-col gap-1">
          ...
          <select className="border border-gray-200 rounded-lg px-3 py-2 text-sm"
            value={form.pole} onChange={e => set('pole', e.target.value)}>
            {POLES.map(p => <option key={p}>{p}</option>)}
          </select>
        </div>
      ...
      <div className="flex gap-2 justify-between mt-5 pt-4 border-t border-gray-100">
        <button onClick={() => onDelete(eleve.id)}
          className="text-xs bg-red-50 text-red-600 hover:bg-red-100 px-4 py-2 rounded-lg">Supprimer</button>
        <div className="flex gap-2">
          <button onClick={() => onSave(form)}
            className="text-xs bg-[#2d6a2d] text-white hover:bg-[#4a8c4a] px-4 py-2 rounded-lg">Enregistrer</button>
        </div>
      </div>
    </div>
  )
